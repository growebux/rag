...
router.post(
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      try {
        ragResponse = await dataService.queryWithContext(
          message,
          session.context?.section as OnboardingSectionType
        );
...
`;
        ...
        if (session.context?.section) {
          const sectionNames: Record<OnboardingSectionType, string> = {
...
router.get(
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      req.log.error(
        error,
        `Failed to retrieve chat history for session: ${req.params['sessionId']}`
      );
      return res.status(500).json({
        success: false,
...
function generateChatSuggestions(
  ...
  const filteredSuggestions = suggestions.filter(suggestion => {
    ...
    const hasCommonWords = suggestionWords.some(
...
function formatChatResponse(rawResponse: string): string {
