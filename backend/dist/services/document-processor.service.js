...
class DocumentProcessorService {
    chunkSize = 800; // Larger chunks, fewer API calls
    ...
    async processDocument(document) {
        ...
        try {
            ...
            const documentEmbedding = await (0, openai_service_1.openAIService)().generateEmbeddings(`${document.title}\n\n${document.content}`);
            ...
            throw new Error(`Failed to process document ${document.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
    /**
     ...
     */
    async createDocumentChunks(document) {
        const chunks = [];
        ...
        if (content.length <= this.chunkSize) {
            ...
            const embedding = await (0, openai_service_1.openAIService)().generateEmbeddings(content);
        ...
        while (startIndex < content.length) {
            ...
            const chunkPromise = (async () => {
                ...
                const embedding = await (0, openai_service_1.openAIService)().generateEmbeddings(chunkContent);
                const chunkTime = Date.now() - chunkStart;
            ...
            const nextStartIndex = startIndex + chunkContent.length - this.chunkOverlap;
    ...
    validateDocument(document) {
        ...
        if (!Object.values(rag_1.OnboardingSectionType).includes(document.section)) {
            errors.push('Invalid onboarding section type');
        }
        if (document.content && document.content.length > 50000) {
            errors.push('Document content exceeds maximum length of 50,000 characters');
        }
        return {
...
let _documentProcessorService = null;
