...
exports.ragService = exports.RAGServiceImpl = void 0;
...
class RAGServiceImpl {
    ...
    async initialize(documents) {
        ...
        console.log(`[DEBUG] RAG initialization started with ${documents.length} documents`);
        try {
            // Clear existing documents
            const clearStart = Date.now();
            await (0, vector_store_service_1.vectorStore)().clear();
            console.log(`[DEBUG] Vector store cleared in ${Date.now() - clearStart}ms`);
            // Process and add documents
            const processStart = Date.now();
            console.log(`[DEBUG] Starting document processing...`);
            const processedDocuments = await (0, document_processor_service_1.documentProcessorService)().processDocuments(documents);
            const processTime = Date.now() - processStart;
            console.log(`[DEBUG] Document processing completed in ${processTime}ms`);
            const storeStart = Date.now();
            console.log(`[DEBUG] Adding ${processedDocuments.length} documents to vector store...`);
            for (const processedDoc of processedDocuments) {
                await (0, vector_store_service_1.vectorStore)().addDocument(processedDoc);
            }
            const storeTime = Date.now() - storeStart;
            console.log(`[DEBUG] Vector store population completed in ${storeTime}ms`);
            this.isInitialized = true;
            const totalTime = Date.now() - startTime;
            console.log(`[DEBUG] RAG system initialized with ${processedDocuments.length} documents in ${totalTime}ms`);
        }
        catch (error) {
            const totalTime = Date.now() - startTime;
            console.error(`[DEBUG] RAG initialization failed after ${totalTime}ms:`, error);
    ...
    async query(question, context) {
        ...
        try {
            ...
            const queryEmbedding = await (0, openai_service_1.openAIService)().generateEmbeddings(queryText);
            // Find similar documents/chunks
            const similarResults = await (0, vector_store_service_1.vectorStore)().findSimilar(queryEmbedding, this.maxSources * 2);
            ...
            const relevantSources = similarResults
                ...
                .map(result => this.createDocumentSource(result.document, result.chunk, result.similarity));
            if (relevantSources.length === 0) {
                return {
                    answer: "I don't have enough relevant information to answer that question. Could you please rephrase or ask about a specific onboarding section?",
                    sources: [],
                    confidence: 0,
                };
            }
            // Generate contextual answer using OpenAI
            const answer = await this.generateContextualAnswer(question, relevantSources, context);
    ...
    async addDocument(document) {
        try {
            const processedDocument = await (0, document_processor_service_1.documentProcessorService)().processDocument(document);
            await (0, vector_store_service_1.vectorStore)().addDocument(processedDocument);
            if (!this.isInitialized) {
                this.isInitialized = true;
            }
        }
    ...
    async removeDocument(documentId) {
        try {
            await (0, vector_store_service_1.vectorStore)().removeDocument(documentId);
        }
        catch (error) {
     ...
     */
    getDocuments() {
        return (0, vector_store_service_1.vectorStore)().getAllDocuments();
    }
    /**
     * Generate a contextual answer using OpenAI
     */
    async generateContextualAnswer(question, sources, context) {
        const contextualInfo = sources
            .map(source => `From ${source.title} (${source.section}): ${source.excerpt}`)
            .join('\n\n');
...
Based ONLY on the documentation context provided above, please provide a clear, well-structured, and helpful answer using PLAIN TEXT ONLY. Organize your response with simple paragraphs and lists. If the context doesn't contain sufficient information to fully answer the question, explicitly state what information is missing and suggest what additional documentation sections might be helpful.`;
        try {
            const answer = await (0, openai_service_1.openAIService)().generateResponse(systemPrompt, []);
     ...
     */
    formatResponseForFrontend(rawResponse) {
     ...
     */
    createDocumentSource(document, chunk, similarity) {
...
exports.RAGServiceImpl = RAGServiceImpl;
